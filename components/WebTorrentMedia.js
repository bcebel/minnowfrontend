...
export default function WebTorrentMedia({ media, isFocused }) {
  const debugLog = (stage, data = {}) => {
    ...
    console.log(
      `[${timestamp}] WebTorrentMedia [${cid?.slice(0, 8)}...] ${stage}`,
      {
        ...data,
...
const loadViaWebTorrent = async () => {
  ...
  // Use the service instead of window.globalWebTorrentClient
...
if (!torrent) {
  ...
  // Use the service's add method which includes trackers
