...
const DynamicMediaRenderer: React.FC<DynamicMediaRendererProps> = ({ media, isFocused }) => {
  ...
  useEffect(() => {
    ...
    // Stricter URL selection logic to prevent passing non-video URLs to the video player
    ...
    // Use ipfsUrl as a fallback if the specific URL is not present
    if (!sourceUrl) {
        sourceUrl = media.ipfsUrl;
    }

    const url = getPinataUrl(sourceUrl);
    setMediaUrl(url);
  }, [media]);
